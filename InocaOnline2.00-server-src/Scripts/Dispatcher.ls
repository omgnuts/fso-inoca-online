...
-- global TreasureList, MoveProp, MapPropList, Dispt,           -- This object
global gDispatcher, gMovie, gServer
global gClientVersion
-- global gMovementQueue,
------------------------------------------------------------------------------------------
...
on initialize (me, aUser, aServer)
  ...
  -- gMovementQueue = ""
  gClientVersion = string(file("DAT\SETTINGS\ClientVersion.dam").read)
  ...
  pServer.abortCheckPeriod = 8000
	...
	gMovie = aServer.createServerMovie("inomov")

	initializeGlobalConstants()
	refreshGlobals()

	-- listeners for time where 1000 = 1sec
	timeout("TimeOutHalfSec").new(500, #RunTimeOutHalfSec, me)
...
--"------------------------------------------------------
-- // Check if the user is logging into a valid movie
--------------------------------------------------------
-- If the movielist is not found, then it is being
...
on serverEvent (me, event, movie, group, user)
  ...
  case event of
   #userLogon:
		  ...
		  resetTimeStamp(user.name)
			-- the real userLogon is done after user password / account is checked
   #userLogoff:
			userLogOffa(user.name, movie)
  end case
...
on incomingMessage (me, recipientID, subject, senderID, errorCode, timeStamp, msg, sender, isUDP)
  ...
  --put "LingoVM incomingMessage recipientID: " & recipientID & " subject:" & subject & " senderID:" & senderID & " errorCode:" & errorCode & " timeStamp:" & timeStamp & " sender:" & sender & " udp:" & isUDP
	...
	if string(sender.name) <> string(senderID) then
		removeUserSession(movie, sender.name, "bad_session")
		exit
	end if
