...
on WarpShortcuts(WarpDat, user, movie)

	set the itemdelimiter = " "
	set CharToWarp =  item 2 of string(WarpDat)
	set WarpPlace =  item 4 of string(WarpDat)
	set WarpPlace = file("DAT\Warpshortcuts\" & WarpPlace & ".txt").Read
	if WarpPlace contains "x" then
		set NewWarpDat = "/warp " & CharToWarp & " " & WarpPlace
		return NewWarpDat
	else
		sendMovieMessage(movie, user.name, "sqa", "That shortcut doesn't exist")
		return "NoFile"
	end if
